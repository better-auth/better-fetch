...
export function getTimeout(
	...
	// a caller `signal` no longer disarms the timer: the request listens to
	// `controller`, and a forwarded caller signal aborts it too, so whichever
	// fires first wins instead of the signal silently cancelling the deadline.
	if (options?.timeout) {
